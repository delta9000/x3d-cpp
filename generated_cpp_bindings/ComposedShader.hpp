// ComposedShader.hpp
#ifndef COMPOSEDSHADER_HPP
#define COMPOSEDSHADER_HPP
 ...
 * @brief ComposedShader can contain field declarations, but no CDATA section of
 * plain-text source code, since programs are composed from child ShaderPart
 * nodes.
 * @details
 * https://www.web3d.org/specifications/X3Dv4/ISO-IEC19775-1v4-IS/Part01/components/shaders.html#ComposedShader
 */
class ComposedShader : public virtual X3DShaderNode,
                       public virtual X3DProgrammableShaderObject {
public:
  /**
   * @brief Default constructor for ComposedShader
   */
  ComposedShader() = default;

  /**
   * @brief Destructor for ComposedShader
   */
  ~ComposedShader() = default;
   ...
   * @details Permitted X3D node types: ShaderPart
   * @return const std::vector<std::string>& The accepted node type names.
   */
  static const std::vector<std::string> &acceptablePartsNodeTypes() {
    static const std::vector<std::string> types = {"ShaderPart"};
    return types;
  }

  /**
   * @brief Sets the value of parts. AccessType: inputOutput
   * @details ComposedShader can contain multiple ShaderPart nodes in the parts
   * field.
   * @param value The new value for parts.
   */
  void setParts(const MFNode &value) { _parts = value; }

  void setParts(MFNode &&value) { _parts = std::move(value); }

  /**
   * @brief The X3D type name of this node (e.g. "ComposedShader").
   */
...
private:
  ...
  /**
   * @brief Member variable for parts.
   */

  MFNode _parts{};
};

#endif // COMPOSEDSHADER_HPP