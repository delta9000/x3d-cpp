...
const FieldTable &PositionInterpolator::fields() const {
  static const FieldTable table = [] {
    ...
    t.push_back(FieldInfo{
        ...
        [](const X3DNode &n) -> FieldView {
          return {&dynamic_cast<const PositionInterpolator &>(n)
                       .X3DInterpolatorNode::getKey(),
                  &typeid(MFFloat)};
        }
    ...
    t.push_back(FieldInfo{
        ...
        [](const X3DNode &n) -> FieldView {
          return {&dynamic_cast<const PositionInterpolator &>(n).getKeyValue(),
                  &typeid(MFVec3f)};
        }
...
std::shared_ptr<X3DNode> createPositionInterpolator() {
  return std::make_shared<PositionInterpolator>();
}
} // namespace factory_detail
