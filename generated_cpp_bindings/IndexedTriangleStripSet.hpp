// IndexedTriangleStripSet.hpp
#ifndef INDEXEDTRIANGLESTRIPSET_HPP
#define INDEXEDTRIANGLESTRIPSET_HPP
 ...
 * @brief IndexedTriangleStripSet is a geometry node containing a
 ...
 */
class IndexedTriangleStripSet : public virtual X3DComposedGeometryNode {
public:
  /**
   * @brief Default constructor for IndexedTriangleStripSet
   */
  IndexedTriangleStripSet() = default;

  /**
   * @brief Destructor for IndexedTriangleStripSet
   */
  ~IndexedTriangleStripSet() = default;
   ...
   * @details index list specifies triangles by connecting Coordinate vertices
   * for each individual strip, separated by -1 sentinel values. Dispatches to
   * the handler registered via setOnSet_indexHandler(); a no-op if none is set.
   * The event cascade reaches this through the node's reflected field table, so
  ...
  /**
   * @brief The X3D type name of this node (e.g. "IndexedTriangleStripSet").
   */
...
#endif // INDEXEDTRIANGLESTRIPSET_HPP