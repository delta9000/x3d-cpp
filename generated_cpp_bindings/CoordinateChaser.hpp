// CoordinateChaser.hpp
#ifndef COORDINATECHASER_HPP
#define COORDINATECHASER_HPP
 ...
 * @brief CoordinateChaser generates a series of coordinate arrays that
 * progressively change from initial value to destination value.
 * @details
 * https://www.web3d.org/specifications/X3Dv4/ISO-IEC19775-1v4-IS/Part01/components/followers.html#CoordinateChaser
 */
class CoordinateChaser : public virtual X3DChaserNode {
public:
  /**
   * @brief Default constructor for CoordinateChaser
   */
  CoordinateChaser() = default;

  /**
   * @brief Destructor for CoordinateChaser
   */
  ~CoordinateChaser() = default;
   ...
   */
  static MFVec3f getDefaultInitialDestination() {
   ...
   * @return MFVec3f The default value
   */
  static MFVec3f getDefaultInitialValue() {
    return std::vector<SFVec3f>{SFVec3f{0, 0, 0}};
  }
   ...
   * @return MFVec3f The current value of initialDestination.
   */
  MFVec3f getInitialDestination() const { return _initialDestination; }
  /**
   ...
   */
  void setInitialDestinationUnchecked(const MFVec3f &value) {
   ...
   * @return MFVec3f The current value of initialValue.
   */
  MFVec3f getInitialValue() const { return _initialValue; }
  /**
   ...
   */
  void setInitialValueUnchecked(const MFVec3f &value) { _initialValue = value; }
   ...
   */
  void onSet_destination(const MFVec3f &value) {
   ...
   */
  void
  setOnSet_destinationHandler(std::function<void(const MFVec3f &)> handler) {
   ...
   * @param value The value of the received event.
   */
  void onSet_value(const MFVec3f &value) {
   ...
   * handler).
   */
  void setOnSet_valueHandler(std::function<void(const MFVec3f &)> handler) {
   ...
   */
  void emitValue_changed(const MFVec3f &value) { _value_changed = value; }

  /**
   * @brief The X3D type name of this node (e.g. "CoordinateChaser").
   */
...
private:
  ...
  MFVec3f _initialDestination{std::vector<SFVec3f>{SFVec3f{0, 0, 0}}};

  /**
   * @brief Member variable for initialValue.
   */

  MFVec3f _initialValue{std::vector<SFVec3f>{SFVec3f{0, 0, 0}}};

  /**
   * @brief Registered event handler for set_destination (inputOnly); empty
   * until set.
   */
  std::function<void(const MFVec3f &)> _onSet_destinationHandler{};

  /**
   * @brief Registered event handler for set_value (inputOnly); empty until set.
   */
  std::function<void(const MFVec3f &)> _onSet_valueHandler{};

  /**
   * @brief Member variable for value_changed.
   */

  MFVec3f _value_changed{};
};

#endif // COORDINATECHASER_HPP