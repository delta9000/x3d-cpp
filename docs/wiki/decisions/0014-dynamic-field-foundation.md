...
- Side-table lifetime is keyed by `const X3DNode*`. As first shipped this was safe only for a single load→tick→extract pass: nothing erased an entry, so the table grew for the life of the process, and a node allocated at a freed node's address inherited its author fields. **Amended 2026-09-25:** readers now register through `addAuthorFields(std::shared_ptr<const X3DNode>, …)`, and the entry tracks its node with a `weak_ptr`. A lookup treats an expired entry as absent, and stale entries are swept on insertion with an amortized doubling threshold. The synthesized get/set thunks hold a `weak_ptr` to their entry, so a `FieldInfo` copy that outlives `erase()`/`clear()` is inert rather than a use-after-free. They take a per-entry mutex, and the set thunk applies the same AUD-MEM-1 type check as `setValue()`. The `const X3DNode&` overloads remain for callers that guarantee the node's lifetime (tests with stack nodes). They register an untracked entry.
