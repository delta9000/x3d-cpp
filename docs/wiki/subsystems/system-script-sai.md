...
- **Call budget** — `ScriptEngine::setCallBudget()` (default 2 s, zero disables) bounds each entry into script code: load, initialize, one invoke, prepareEvents, eventsProcessed, shutdown. A call that runs past it is interrupted (Duktape exec-timeout check; QuickJS interrupt handler), logged as a script error, and the script stays loaded. A load whose top level times out fails. See [ADR-0048](../decisions/0048-script-node-handles.md).
...
- `ctest -R x3d_quickjs_backend` (`runtime/script/tests/quickjs_backend_test.cpp`) — the QuickJS backend's own lifecycle + marshalling + Browser + handler-dispatch tests (mirrors the Duktape T1–T16 surface, including T15b–d, T15f and T15g).
