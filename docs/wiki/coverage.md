...
| covered | `decisions/0046-loadsensor-assetresolver-oracle.md` | `LoadSensor` becomes a live `X3DNetworkSensorNode` via a time-driven `LoadSensorSystem` — the first SDK-side caller of the AssetResolver seam (ADR-0023). Load truth = injected resolver + a parse-time pre-seed of already-expanded Inlines (`Scene::expandedInlines`); per-sensor state lives in the system, never on the node. A Ready-only system-wide URL memo, a ≤1-resolver-call-per-child-per-tick bound made idempotent against the cascade settle loop via `tickGeneration()`, a SEC-3-confined local-file default resolver, and rulings R3–R7 for the spec-silent edges. Closes NSN-1..7, NSN-9 | `runtime/events/LoadSensorSystem.hpp`, `runtime/io/file/FileResolver.hpp`, `runtime/events/tests/loadsensor_test.cpp` |
