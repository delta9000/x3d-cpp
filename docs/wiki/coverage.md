...
Every row is currently `covered`; see the summary below. (The Wave-1 vertical slice originally covered three rows, the [CLI Suite subsystem](subsystems/cli-suite.md), [ADR-0001 Ext Firewall](decisions/0001-ext-firewall.md) and the [Gate System guide](guides/gate-system.md), and later waves filled the rest.)
...
| covered | `subsystems/system-loadsensor.md` | LoadSensor as a live X3DNetworkSensorNode: a time-driven System over the AssetResolver seam observing child URL-object load state per tick (closes NSN-1..7, NSN-9) | `runtime/events/LoadSensorSystem.hpp`, `runtime/io/file/FileResolver.hpp` |
...
| covered | `subsystems/asset-import.md` | Asset-import backend seam, texture pipeline, and the footprint-gated authoring target | `examples/asset_import/`, `include/x3d/authoring.hpp`, `scripts/authoring-footprint.sh` |
...
| covered | `decisions/0041-moviedecoder-seam-royalty-free-defaults.md` | Movie/video playback split at the SDK boundary: a `MovieDecoder` seam with royalty-free defaults (plmpeg) | `runtime/io/plmpeg/` |
...
| covered | `decisions/0046-loadsensor-assetresolver-oracle.md` | `LoadSensor` becomes a live `X3DNetworkSensorNode` via a time-driven `LoadSensorSystem` — the first SDK-side caller of the AssetResolver seam (ADR-0023). Load truth = injected resolver + a parse-time pre-seed of already-expanded Inlines (`Scene::expandedInlines`); per-sensor state lives in the system, never on the node. A Ready-only system-wide URL memo, a ≤1-resolver-call-per-child-per-tick bound made idempotent against the cascade settle loop via `tickGeneration()`, an IO-free null default resolver (the app injects the SEC-3-confined local-file backend, keeping the SDK IO-free), and rulings R3–R7 for the spec-silent edges. Closes NSN-1..7, NSN-9 | `runtime/events/LoadSensorSystem.hpp`, `runtime/io/file/FileResolver.hpp`, `runtime/events/tests/loadsensor_test.cpp` |
| covered | `decisions/0047-sai-sister-repo-split.md` | The experimental modern-C++ SAI semantic kernel moves to the private `x3d-sai` sister repository with a self-contained generator: the X3D UOM is a Web3D artifact, so both projects vendor and SHA-pin it independently rather than x3d-sai depending on `x3d-cpp-gen`. Accepted cost: ~1,650 LOC of parse/model/descriptor logic duplicated, making re-entry a reconciliation. The cross-implementation service register splits 29 x3d-cpp rows / 36 kernel rows; the UOM reflection parity check returns here over all 330 concrete nodes. Re-entry trigger: `sai-conformance` (strict) going green in x3d-sai | `tests/test_uom_runtime_reflection_parity.py`, `docs/conformance/sai-services.yaml`, `runtime/script/SaiContext.hpp` (unaffected) |
