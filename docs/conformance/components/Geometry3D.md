...
- **VIS-EXTRUSION-EMPTY** [major/OPEN] — §13.3.5: Extrusion with default/simple fields extracts NO geometry (render_items=0) — default_extrusion.x3d and test_simpleorient.x3d render blank (reproduces in cpu_raster too, so it is an SDK extraction gap, not poc-specific).
  - Verified render_items=0 via poc --headless on ConformanceNist Geometry/Extrusion/{default_extrusion,test_simpleorient}. Distinct from EXTRUSION-SCP (cross-section ORIENTATION, not zero output). tessellateExtrusion (MeshBuilder.hpp) emits nothing for these inputs — root-cause the cull (likely the <2-distinct-spine-point guard or default closed crossSection handling) for a default crossSection + straight spine.
- **VIS-ELEV-SPACING-EMPTY** [major/OPEN] — §13.3.4: ElevationGrid with non-default xSpacing/zSpacing extracts NO geometry (render_items=0) — test_xSpacing.x3d, test_zSpacing.x3d render blank, while test_color (default spacing) extracts fine.
  - Verified render_items=0 via poc --headless on ConformanceNist Geometry/ElevationGrid/{test_xSpacing,test_zSpacing}. Distinct from EXT-003 (winding, fixed) and EXT-001 (color/normal ignored). The spacing fields specifically zero the output — check height-array length vs xDimension/zDimension sizing / guard in emitHeightGrid().
