...
- **SCRIPT-EVENTIN** [major/OPEN] — §29.2; 4.4.8.3: A ROUTE into a Script eventIn/inputOutput (or set_url) is validated and registered but NEVER dispatched at tick time — the canonical sensor->Script->scene pattern is inert in production.
  - X3DEventCascade.hpp:185-198 deliver() scans the STATIC fields() table; Script author fields live only in effectiveFields()/the dynamic store, so the lookup misses and the event evaporates with no diagnostic (buildRoutes resolved the sink via effectiveFields, X3DSceneBridge.hpp:74). Same asymmetry silently drops `set_url` ROUTEs. eventsProcessed() also never fires in production (gated on receivedEventThisTick). Fix: resolve delivery through effectiveFields() and forward Script-field writes to ScriptSystem::deliverInputEvent. (event-model review; empirically confirmed — `x3d sim` on a TouchSensor ROUTE produces nothing.)
...
- **ENC-CDATA-SCRIPT** [minor/OPEN] — §ISO 19776-1 (CDATA); 19775-1 29: Script source containing the literal `]]>` is silently truncated (`if (a]]>b)` -> `if (ab)`) — the XML writer wraps source in one CDATA block without splitting.
  - XmlWriter wraps el.text as `<![CDATA[ + text + ]]>` with no split; a `]]>` in JS source closes the CDATA early. Partly an inherent XML limit, but the toolchain neither rejects nor preserves — it eats characters. Fix: split on `]]>` into consecutive CDATA sections (`]]]]><![CDATA[>`). (encoding review; confirmed by round-trip sweep.)
