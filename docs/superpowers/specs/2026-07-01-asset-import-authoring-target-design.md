...
### Why we own the X3D emit (rather than reuse assimp's `X3DExporter`)

Assimp includes an `X3DExporter`, but its output targets X3D 3.x conventions
(`IndexedFaceSet`/`Material`, single XML encoding) and predates X3D 4.0's PBR material model and
this project's conformance tooling. We therefore use assimp **only as a front-end importer** (read
40+ formats into the IR) and own the X3D emit ourselves — the emit is where this repo's
conformance work lives, and it is the reason the consumer earns its place. What our emit adds by
owning it: X3D 4.0 PBR (`PhysicalMaterial`), range-validation + profile-fit + optional round-trip
(we only write conformant output), canonical form + all four encodings from one graph,
narrowest-profile declaration, texture asset handling, and `DEF`/`USE` instancing.
...
- **`FixtureSource`** — a tiny dependency-free backend producing a handful of synthetic scenes
  ...
  assimp**, and it is the interim *second independent backend* that keeps the seam honest in v1.

**Committed fast-follow: a native glTF backend** (`GltfSource`, via cgltf/tinygltf). Routing
everything through assimp's `aiScene` IR is itself lossy for modern PBR — assimp normalizes PBR
awkwardly and **drops most glTF `KHR_*` extensions on import**. A direct glTF→IR backend behind
this same `ImportSource` interface is therefore the **high-fidelity PBR path** for the format
that matters most, with no assimp normalization in the middle. It also upgrades `ImportSource`
from "fixture is the second backend" into a real **two-independent-backend U1–U4 genericity
proof**. Sequenced immediately after the assimp path lands (see §11). (USD stays deferred:
assimp's USD support is thin/experimental via bundled `tinyusdz`; a first-class USD backend would
be a dedicated OpenUSD/`tinyusdz` implementation behind this same interface, not via assimp.)
...
### 6.1 Relationship to ISO/IEC 19777-4 (the X3D C++ binding)

ISO/IEC 19777-4 is the X3D C++ language binding (published 2025, binding X3D v3.3; a v4.0-aligned
update is 2026 work). It targets the SAI runtime/browser access model and follows the
transliteration style of the Java binding (19777-2). `x3d_cpp::authoring` targets a different use
— headless, embedded, offline authoring — and adopts contemporary C++ idioms accordingly. The
table contrasts the two so our choices are deliberate and forward-compatible:

| Axis | 19777-4 (2025 IS + reference impl) | `x3d_cpp::authoring` (ideal shape) |
|---|---|---|
| Structure | UOM-autogenerated (2026 target); two-tier abstract interfaces (Annex A) + concrete classes (Annex B), one per node | **Same two-tier, UOM-derived generation** — deliberately isomorphic so conformance is later a thin adapter |
| Namespace / names | global namespace; `C`/`CX3D` class prefixes; `X3DLib`; `Concretes.h`/`Abstracts.h`; family acronym **`X3DCSAIL`** | neutral `x3d::…`, plain node names (`Shape`, `Material`); **claims none of those identifiers** |
| Field access | `getFoo`/`setFoo`; tuples as bare `float*`; MF as `(ptr, int size)` + `getNumFoo()`; `set1Value()` | typed accessors over `std::vector`/typed value structs; keep get/set semantics; a `set1Value`-equivalent |
| Ownership / lifetime | raw pointers + Java-style `dispose()` + throw-after-dispose | **RAII + explicit ownership** (nodes owned by parent/document), value-semantic field data; **no `dispose()`** |
| Errors | mandatory exceptions, Java taxonomy (`InvalidNodeException`…) | **survives `-fno-exceptions`**: diagnostic/status returns on the authoring+validate path (reuse `RangeDiagnostic`); exceptions only at an optional ergonomic edge |
| Packaging | compiled lib; reference impl is MFC/Windows, RTTI-dependent (`DECLARE_DYNAMIC`) | portable, headless, no MFC, no RTTI requirement — the slim static lib |
| Serialization | `toXMLString()` (XML only in the impl) | **all four encodings** (XML/JSON/VRML/Canonical) — a superset |

**Forward-compat strategy.** Keep our generated node model *structurally isomorphic* to the UOM
two-tier model (our bindings already are, being UOM-derived), so a future 19777-4 conformance
layer is a **thin adapter** (accessor renames + a `dispose()`/exception facade), not a core
change. Treat 19777-4 as **informative, not normative**: the 2025 IS binds X3D **v3.3**, and the
**v4.0**-aligned binding is **2026** work still in progress — so our target tracks it rather than
conforms to it for now.

**Naming guardrails (do not squat).** Avoid `X3DCSAIL`, `X3DCPPSAIL`, the `C`/`CX3D` class-name
prefixes, `X3DLib`, `Concretes.h`/`Abstracts.h`, and the `web3d::`/`org.web3d.x3d` namespace
lineage. Reuse spec-mandated *binding* identifiers (`dispose`, the exception taxonomy,
`beginUpdate`/`endUpdate`) only inside a future conformance adapter, never in the core. Abstract
SAI *data-type* names (`SFVec3f`, `SFColor`) are generic and fine.

**Optional authoring-conformance oracle (offline tier, documented fast-follow).** X3DJSAIL is a
mature, UOM-generated reference and already this repo's golden oracle (`canon-golden-gen`). We can
differentially test the authoring path by building the same scene in our SAIL and in X3DJSAIL and
diffing the **canonical** serialization — language-agnostic, so binding-naming-independent. Same
offline tier as the existing canon/cli goldens (needs JDK), not a core-CI hard gate.

**ADR.** The deliberate divergence from 19777-4 (and the naming guardrails) is a binding design
decision → capture it as an ADR alongside the `x3d_cpp::authoring` boundary ADR.
...
- New example/subsystem page (`docs/wiki/subsystems/asset-import.md`) + `coverage.md` row +
  mkdocs nav.
- **ADR** for the `x3d_cpp::authoring` boundary + the deliberate divergence from ISO/IEC 19777-4
  and the naming guardrails (§6.1).
...
**Committed fast-follow (next card after v1):** the native `GltfSource` backend (§3) — the
high-fidelity PBR path and the real second backend that turns `ImportSource` into a full U1–U4
genericity proof.

**Fidelity ceiling (documented, backend-independent):** X3D 4.0 has no node for several glTF
`KHR_*` extensions (transmission, clearcoat, sheen, volume, …). Even the native glTF backend
cannot represent these; we emit the representable subset (base/metallic/roughness/normal/
occlusion/emissive) and record the drop-list in the subsystem page rather than imply loss-free
conversion.
...
textures. Behavioral/spec gaps land in `docs/conformance/findings.yaml`; engineering deferrals on
GitHub Project #2.
