...
std::string ClassicVrmlReader::mapNodeName(const std::string &token) const {
  return token;
}

std::string ClassicVrmlReader::mapFieldName(const std::string & /*nodeType*/,
                                            const std::string &token) const {
  return token;
}

void ClassicVrmlReader::onHeaderLine(std::string_view /*src*/,
                                     runtime::X3DDocument & /*doc*/) {}

void ClassicVrmlReader::warn(const std::string & /*message*/) {}

std::string_view ClassicVrmlReader::firstLine(std::string_view src) {
...
void ClassicVrmlReader::parseHeaderLine(std::string_view src,
                                        runtime::X3DDocument &doc) {
...
void ClassicVrmlReader::parseHeaderStatements(VrmlTokenizer &tok,
                                              runtime::X3DDocument &doc) {
  for (;;) {
...
void ClassicVrmlReader::parseSceneBody(VrmlTokenizer &tok,
                                       runtime::Scene &scene, bool inProto,
...
ClassicVrmlReader::parseNode(VrmlTokenizer &tok, runtime::Scene &scene,
                             runtime::ProtoBody *currentProtoBody,
                             const std::shared_ptr<X3DNode> &parentNode,
                             const std::string &parentField) {
  NestingGuard guard(depth_,
                     "ClassicVRML"); // SEC-1: bound recursive node nesting.
  std::string def;
  ...
  if (tok.peek().isPunct('}')) {
    ...
    warn("missing '}' closing " + typeName + " node body at line " +
         std::to_string(tok.peek().line) +
...
void ClassicVrmlReader::mirrorScriptSource(X3DNode &node) {
...
std::string ClassicVrmlReader::decodeInlineScriptSource(const MFString &url) {
...
void ClassicVrmlReader::parseNodeBody(
    VrmlTokenizer &tok, runtime::Scene &scene, X3DNode &node,
    const std::string &nodeType, runtime::ProtoBody *currentProtoBody,
  ...
  for (;;) {
    ...
    std::string_view canonicalName = canonicalInputFieldName(
        nodeType.empty() ? node.nodeTypeName() : nodeType, name);
...
void ClassicVrmlReader::applyNodeField(
    VrmlTokenizer &tok, runtime::Scene &scene, X3DNode &parent,
    const FieldInfo &f, runtime::ProtoBody *currentProtoBody,
    const std::shared_ptr<X3DNode> &parentShared) {
  ...
  if (f.type == X3DFieldType::SFNode) {
    ...
    if (tok.peek().isPunct('[') || tok.peek().kind == VrmlToken::Kind::Number ||
        tok.peek().kind == VrmlToken::Kind::String) {
      skipFieldValue(tok);
      return;
    }
...
void ClassicVrmlReader::parseImport(VrmlTokenizer &tok, runtime::Scene &scene) {
  tok.next(); // IMPORT
  runtime::Import imp;
  if (tok.peek().isPunct('{')) {
...
void ClassicVrmlReader::parseExport(VrmlTokenizer &tok, runtime::Scene &scene) {
  tok.next(); // EXPORT
  runtime::Export exp;
  if (tok.peek().isPunct('{')) {
...
void ClassicVrmlReader::parseProtoInstance(VrmlTokenizer &tok,
                                           runtime::Scene &scene,
...
bool ClassicVrmlReader::nextIsNodeValue(VrmlTokenizer &tok) {
  const VrmlToken &t = tok.peek();
  if (t.isWord("DEF") || t.isWord("USE") || t.isWord("NULL") || t.isPunct('['))
    return true;
...
bool ClassicVrmlReader::skipNonNodeListItem(VrmlTokenizer &tok,
  ...
  if (t.kind == VrmlToken::Kind::Number || t.kind == VrmlToken::Kind::String ||
      t.isPunct('[')) {
...
void ClassicVrmlReader::skipBalancedBraceBlock(VrmlTokenizer &tok) {
...
void ClassicVrmlReader::skipFieldValue(VrmlTokenizer &tok) {
  const VrmlToken &t = tok.peek();
...
void ClassicVrmlReader::skipRestOfBraceBlock(VrmlTokenizer &tok) {
  int depth = 1;
  while (!tok.atEnd() && depth > 0) {
    const VrmlToken t = tok.next();
    if (t.isPunct('{'))
...
void ClassicVrmlReader::captureInstanceNodeValue(VrmlTokenizer &tok,
                                                 runtime::Scene &scene,
                                                 runtime::ProtoFieldValue &fv,
                                                 X3DFieldType ty) {
...
std::runtime_error ClassicVrmlReader::error(const VrmlToken &t,
...
std::string ClassicVrmlReader::expectWord(VrmlTokenizer &tok,
...
std::string ClassicVrmlReader::expectString(VrmlTokenizer &tok,
                                            const std::string &what) {
  const VrmlToken &t = tok.peek();
  if (t.atEnd())
    throw error(t, "expected " + what + ", got end of input");
...
void ClassicVrmlReader::expectPunct(VrmlTokenizer &tok, char c,
                                    const std::string &what) {
  const VrmlToken &t = tok.peek();
  if (!t.isPunct(c))
...
bool ClassicVrmlReader::hasDefaultValueAhead(VrmlTokenizer &tok) {
  const VrmlToken &t = tok.peek();
  if (t.atEnd() || t.isPunct(']') || t.isPunct('}'))
    return false;
...
bool ClassicVrmlReader::isAccessTypeWord(const std::string &w) {
  return w == "inputOnly" || w == "outputOnly" || w == "initializeOnly" ||
         w == "inputOutput" || w == "eventIn" || w == "eventOut" ||
         w == "field" || w == "exposedField";
}

AccessType ClassicVrmlReader::accessTypeFromString(const std::string &w) {
...
X3DFieldType ClassicVrmlReader::fieldTypeFromString(const std::string &w) {
  static const struct {
    const char *name;
    X3DFieldType type;
  } kMap[] = {
...
ClassicVrmlReader::findExternProto(const runtime::Scene &scene,
                                   const std::string &name) {
...
ClassicVrmlReader::findProtoField(const runtime::ProtoInstance &inst,
                                  const std::string &name) {
