...
std::string JsonWriter::writeDocument(const runtime::X3DDocument &doc) {
  seen_.clear();
  defaults_.clear();
  scene_ = &doc.scene;
...
X3DNode *JsonWriter::defaultFor(const std::string &typeName) {
...
void JsonWriter::pad(std::ostringstream &os, int depth) {
  for (int i = 0; i < depth; ++i)
    os << "  ";
}

std::string JsonWriter::jstr(const std::string &s) {
  std::string out = "\"";
  for (char c : s) {
    switch (c) {
    case '"':
...
void JsonWriter::writeHead(std::ostringstream &os, const runtime::Head &head,
                           int depth) {
  bool first = true;
...
void JsonWriter::writeSceneChildren(std::ostringstream &os,
...
void JsonWriter::writeSceneStatements(std::ostringstream &os,
                                      const runtime::Scene &scene, int depth) {
...
JsonWriter::interfaceField(const runtime::ProtoInstance &src,
...
void JsonWriter::writeJsonProtoInstance(std::ostringstream &os,
...
std::string JsonWriter::jsonProtoField(const runtime::ProtoField &f,
...
void JsonWriter::writeJsonExternProtoDeclare(
...
void JsonWriter::writeNode(std::ostringstream &os,
...
std::string JsonWriter::jsonIsBlock(const std::shared_ptr<X3DNode> &node) {
  if (!bodyIsc_)
    return "";
...
JsonWriter::jsonNestedFor(const std::shared_ptr<X3DNode> &node, int depth) {
...
std::vector<std::string>
JsonWriter::jsonSceneNestedFor(const std::shared_ptr<X3DNode> &node,
                               int depth) {
  std::vector<std::string> out;
  if (!scene_)
    return out;
...
std::string JsonWriter::jsonValueOf(const X3DNode &node, const FieldInfo &f) {
  return jsonValue(node, f);
}

std::string JsonWriter::jsonValue(const X3DNode &node, const FieldInfo &f) {
  if (f.isEnum()) {
    if (!f.getEnumString)
      return "";
...
std::string JsonWriter::jsonNodeField(const std::shared_ptr<X3DNode> &node,
...
std::string JsonWriter::jsonScriptSource(const X3DNode &node) {
