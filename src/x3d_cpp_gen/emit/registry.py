...
def gen_interface_registry_source(nodes: Dict[str, X3DNode],
    ...
    lines.append("    // Built once from the table; the immediately-invoked lambda's result")
    lines.append("    // initializes a function-local static, whose initialization C++11")
    lines.append("    // guarantees is thread-safe (no unsynchronized mutable cache).")
    lines.append("    static const std::unordered_map<InterfaceId, std::vector<std::string>> byIface = [] {")
    lines.append("        std::unordered_map<InterfaceId, std::vector<std::string>> m;")
    lines.append("        for (const auto& [name, ifaces] : table())")
    lines.append("            for (InterfaceId i : ifaces)")
    lines.append("                m[i].push_back(name);")
    lines.append("        for (auto& entry : m)")
    lines.append("            std::sort(entry.second.begin(), entry.second.end());")
    lines.append("        return m;")
    lines.append("    }();")
    lines.append("    static const std::vector<std::string> empty;")
    lines.append("    auto it = byIface.find(iface);")
    lines.append("    return it == byIface.end() ? empty : it->second;")
    lines.append("}")
    lines.append("")
    lines.append("} // namespace x3d::nodes")
    lines.append("")
